...
sed -i 's/cluster.broadcast.methods=jgroups/cluster.broadcast.methods=jgroups/g' /opt/hybris/bin/platform/project.properties
sed -i 's/cluster.nodes.autodiscovery=false/cluster.nodes.autodiscovery=true/g' /opt/hybris/bin/platform/project.properties
sed -i 's/<UDP/<UDP bind_addr="match-interface:eth0,match-interface:en0,site_local,loopback"/g' /opt/hybris/bin/platform/ext/core/resources/jgroups-udp.xml
